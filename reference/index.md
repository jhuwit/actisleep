# Package index

## All functions

- [`acti_asleep()`](https://jhuwit.github.io/actisleep/reference/acti_asleep.md)
  : Estimate sleep with the asleep model
- [`acti_sleep_diary()`](https://jhuwit.github.io/actisleep/reference/acti_sleep_diary.md)
  : Identify a diary-defined sleep window
- [`acti_sleep_ensemble()`](https://jhuwit.github.io/actisleep/reference/acti_sleep_ensemble.md)
  : Run a sleep-label ensemble from raw or epoch data
- [`acti_sleep_fuse_labels()`](https://jhuwit.github.io/actisleep/reference/acti_sleep_fuse_labels.md)
  : Fuse multiple sleep/wake labels with STAPLE
- [`acti_sleep_guider()`](https://jhuwit.github.io/actisleep/reference/acti_sleep_guider.md)
  : Dispatch a sleep-period-time guider from data
- [`acti_sleep_guider_requirements()`](https://jhuwit.github.io/actisleep/reference/acti_sleep_guider_requirements.md)
  : Describe data requirements for sleep guiders
- [`acti_sleep_hdcza()`](https://jhuwit.github.io/actisleep/reference/acti_sleep_hdcza.md)
  : Identify a sleep window with HDCZA
- [`acti_sleep_hlrb()`](https://jhuwit.github.io/actisleep/reference/acti_sleep_hlrb.md)
  : Identify the longest rest bout from sustained inactivity bouts
- [`acti_sleep_horangle()`](https://jhuwit.github.io/actisleep/reference/acti_sleep_horangle.md)
  : Identify a sleep window from horizontal posture
- [`acti_sleep_l5()`](https://jhuwit.github.io/actisleep/reference/acti_sleep_l5.md)
  : Identify a sleep window from the least-active five hours
- [`acti_sleep_label()`](https://jhuwit.github.io/actisleep/reference/acti_sleep_label.md)
  : Convert a sleep guider to epoch-level sleep/wake labels
- [`acti_sleep_labels()`](https://jhuwit.github.io/actisleep/reference/acti_sleep_labels.md)
  : Create sleep/wake labels for multiple guiders
- [`acti_sleep_notworn()`](https://jhuwit.github.io/actisleep/reference/acti_sleep_notworn.md)
  : Identify a low-activity window for no-night-wear protocols
- [`acti_sleep_setwindow()`](https://jhuwit.github.io/actisleep/reference/acti_sleep_setwindow.md)
  : Identify a fixed daily sleep window
- [`acti_sleep_sib()`](https://jhuwit.github.io/actisleep/reference/acti_sleep_sib.md)
  : Detect sustained inactivity bouts (SIBs)
- [`acti_sleep_tudor_locke()`](https://jhuwit.github.io/actisleep/reference/acti_sleep_tudor_locke.md)
  : Summarize sleep periods with Tudor–Locke
- [`acti_sleep_tudor_locke_diary()`](https://jhuwit.github.io/actisleep/reference/acti_sleep_tudor_locke_diary.md)
  : Apply a sleep diary to epoch data for Tudor–Locke
- [`acti_sleeper()`](https://jhuwit.github.io/actisleep/reference/acti_sleeper.md)
  : Estimate sleep with the sleeper model
- [`py_acti_asleep()`](https://jhuwit.github.io/actisleep/reference/py_acti_asleep.md)
  : Estimate sleep with asleep in an isolated Python process
- [`py_acti_sleeper()`](https://jhuwit.github.io/actisleep/reference/py_acti_sleeper.md)
  : Estimate sleep with sleeper in an isolated Python process
